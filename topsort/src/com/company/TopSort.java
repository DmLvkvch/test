...
public class TopSort {
    ...
    private String stepDFS() {
        ...
        if (state.nextChild == 0) { // не вернулись в вершину из следующей, а зашли из предыдущей
            ...
            if(list.size()>1){
                graph.getEdges().add(new Edge(list.get(list.size()-2), list.get(list.size()-1), YELLOW));
                if(graph.getEdges().size()>1){
                    for(int i = 0;i<graph.getEdges().size()-1;i++) {
                        if(graph.getEdges().get(i).c!=BROWN)
                            graph.getEdges().get(i).c = BLUE;
                    }
                }
            }
